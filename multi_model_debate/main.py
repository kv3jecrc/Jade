...
main.py
=======
CLI entry point for the Multi-Model Interaction System.

Usage:
    python main.py --topic "Should companies mandate return-to-office?"
    python main.py --topic "..." --mock              # no network calls, for testing
    python main.py --topic "..." --output result.json
    python main.py --topic "..." --verbose            # also log full prompts to console
...
from orchestrator import MultiModelOrchestrator, setup_logging


def main() -> None:
    parser = argparse.ArgumentParser(description="Run a structured multi-model discussion on a topic.")
    parser.add_argument("--topic", type=str, required=True, help="The topic for the models to discuss.")
    ...
    orchestrator = MultiModelOrchestrator(client_a, client_b, orchestration_config)
    result = orchestrator.run_debate(args.topic)
